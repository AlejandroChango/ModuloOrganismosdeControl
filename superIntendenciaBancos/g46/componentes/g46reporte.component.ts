...
@Component({
  selector: 'app-g46Reporte',
  templateUrl: 'g46Reporte.html'
})
export class g46ReporteComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  constructor(public localStorageService: LocalStorageService, router: Router, dtoServicios: DtoServicios) {
    super(localStorageService, router, dtoServicios, '', 'REPORTEG46', false);
  ...
  public generarReporte() {
    this.rqConsulta.CODIGOCONSULTA = 'OC_G46REPORTE';
    this.rqConsulta.storeprocedure = "sp_OdCRptReporteG46";
    ...
    this.dtoServicios.ejecutarConsultaRest(this.rqConsulta)
      .subscribe(
        resp => {
          ...
          this.lregistros = resp.OC_G46REPORTE;
        },
        error => {
  ...
  imprimir(): void {
    ...
    for (const i in this.lregistros) {
      if (this.lregistros.hasOwnProperty(i)) {
        ...
        if (reg.identificacionSujeto !== undefined && reg.identificacionSujeto !== null) {
          ...
          data +=
          reg.tipoIdentificacionSujeto + separador +
          reg.identificacionSujeto + separador +
          reg.numeroOperacion + separador +
          reg.tipoCredito + separador +
          reg.estadoOperacion + separador +
          reg.situacionOperacion + separador +
          reg.destinoGeograficoProv + separador +
          reg.destinoGeograficoCant + separador +
          reg.destinoGeograficoParr + separador
          + reg.fechaConcesion + separador +
          reg.fechaVencimiento + separador +
          reg.valorOperacion + separador +
          reg.tasaInteresNominal + separador +
          reg.periodicidadPago + separador +
          reg.frecuenciaRevision + separador +
          reg.garantesGarantias + salto;
          contador++;
        }
      }
    }
    ...
    var cabecera = 'G46' + separador + codigoSB + separador + fechaUltimoDia + '/' + m + '/' + this.mfiltros.fperiodo;
    ...
    linkElement1.download = "G46" + "M" + codigoSB + fechaUltimoDia + m + this.mfiltros.fperiodo + '.' + "txt";
